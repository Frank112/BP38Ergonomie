...
BodyPostureView::BodyPostureView(QWidget *parent) :
    ...
    vcTrunkTilt(new ValueControl()),
    vcTrunkSidewaysTilt(new ValueControl()),
    vcTrunkTwist(new ValueControl()),
    vcUpperArmAngle(new ValueControl()),
    vcForearmAngle(new ValueControl()),
    vcUpperArmTwist(new ValueControl()),
    vcForearmTwist(new ValueControl()),
    vcArmOpening(new ValueControl()),
    vcWristAngle(new ValueControl()),
    vcWristAngleSideways(new ValueControl()),
    vcWristMovement(new ValueControl()),
    vcHipAngle(new ValueControl()),
    vcHipAngleSideways(new ValueControl()),
    vcHipTwist(new ValueControl()),
    vcKneeAngle(new ValueControl()),
    vcAnkleAngle(new ValueControl()),
    vcAnkleAngleSideways(new ValueControl()),
    vcHeadTilt(new ValueControl()),
    vcHeadTiltSideways(new ValueControl()),
    vcHeadTwist(new ValueControl()),
...
void BodyPostureView::setBodyPosture(QHash<QString, QVariant> values){
    ...
    if((values.value(DBConstants::COL_BODY_POSTURE_UPPER_ARM_ANGLE_RIGHT).toInt() == 90) && (values.value(DBConstants::COL_BODY_POSTURE_FOREARM_ANGLE_RIGHT).toInt() == 180)){
        quickSelectionLayout->voscQuickArmPosture->setSelectedValue(2);
    }

    if((values.value(DBConstants::COL_BODY_POSTURE_UPPER_ARM_ANGLE_RIGHT).toInt() == 135) && (values.value(DBConstants::COL_BODY_POSTURE_FOREARM_ANGLE_RIGHT).toInt() == 180)){
        quickSelectionLayout->voscQuickArmPosture->setSelectedValue(3);
    }
    //QuickLegs
    hipAngleLeft = values.value(DBConstants::COL_BODY_POSTURE_HIP_ANGLE_LEFT).toInt();
